...
layout: post
title: "T1 string"
archive_title: "2021.11.09Simulation"
section: "OI"
section_slug: "oi"
...
summary: "一道构造题。大概是这样的，首先 k 等于 1 的时候，如果 1 在第 i 个位置那么答案就是 i(n-i+1) ，如果 k 等于 2，两个 1 分别在 i j然后 i < j 。那么答案就是 (-i+j)(n+i-j+1) ，如果 k 等于 3，三个 1 分别在 i, j, k 且 i < j < k，那么答案就是 (i-j+k)(n-i+j-k+1)..."
math: true
---
{% raw %}
## T1 string
...
&emsp; 对于询问来说，我们可以对每个区间维护一个 last 数组，last[i] 表示 i 之前的第一个与 i 相同颜色的下标。这样一个区间内颜色互不相同的个数就等价于 $(r-l+1) - \max\limits\_{i=l}^r \lbrace last\_i\rbrace$，这玩意儿就可以用一个 set 和线段树来维护。
