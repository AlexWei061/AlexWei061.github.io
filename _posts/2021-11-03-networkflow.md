...
&emsp; 对于 $(x,y) \in E$ ，$f(x,y)$ 称为这个边的 **流量**，$c(x,y) - f(x,y)$ 称为这条边的**剩余容量**。$\sum\_{(s,v)\in E}f(s,v)$ 表示整个网络的**流量**，其中 s 表示源点。

### 最大流
&emsp; 对于一个网络来说，合法的流函数有很多。其中使得整个网络流量 $\sum\_{(s,v)\in E}f(s,v)$ 最大的流函数称为这个网络的**最大流**，网络最大流时的流量成为网络的最大流量。
