...
summary: "还是那个老 trick 每个数取 乘法变加法，虽然最后还是要打一个高精。"
math: true
---
{% raw %}
## T1 frame
...
&emsp; 首先我们更换枚举方式，先枚举矩形的上界和下界 $a, b$，再枚举左界 $c$，注意左界是从 $m$ 到 $1$ 反着枚举的。考虑能不能 $O(1)$ 找到 右界 $d$，首先我们要求的式子就应该是（记录 $s\_{x, y, a, b}$ 右下角为 $a, b$ 左上角为 $x, y$ 的矩形的和）：
...
&emsp; 为了式子好看一些，我们记 $sum\_x = s\_{a, 1, a, x} + s\_{b, 1, b, x}, xum\_{x} = s\_{a + 1, x, b - 1, x}$，那么现在式子就变成了这样：
...
&emsp; 然后我们发现这里面和 $d$ 有关的式子和 $c$ 的位置无关，所以我们考虑倒着扫 $c$ 然后记录一个 $temp$ 为当前扫过的部分的 $sum\_d + xum\_d$ 的最大值，然后每次 $c$ 往左移就比较一下新的位置的 $sum\_d + xum\_d$ 和 $temp$ 的大小更新这个最大值就做完了。
