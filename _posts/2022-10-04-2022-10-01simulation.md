...
&emsp; 把式子化简一下 $(a_i - 1)(a_j - 1) < 1$，于是就变成了几种情况：

1. $a_j - 1 = 0$，贡献前面的所有数
2. $a_j - 1 < 0$，贡献前面所有满足 $a_i - 1 \geq 0$ 的数
3. $a_j - 1 > 0$，贡献前面所有满足 $a_i - 1 \leq 0$ 的数
...
&emsp; 区间 $dp$，把区间段点离散化，显然不影响答案。然后记录 $f_{l, r}$ 为第  $l$ 个端点到第 $r$ 个端点的颜色最大值，然后就区间 $dp$ 传统艺能，枚举中间点 $k$，并且考虑 $[k, k + 1]$ 涂上了某一个颜色，然后显然这样就必须满足 $\exist$ 区间 $[l_i, r_i]$ 使得 $l \le l_i \le k \le r_i \le r$，如果有这样的区间我们就可以这样转移：

$$ f_{l, k} + f_{k + 1, r} + 1 \to f_{l, r} $$

&emsp; 那么现在考虑怎么做到这个限制条件，这个地方 $std$ 就做的很巧妙了，记录一个 $siz_{l, r}$ 表示 $[l, r]$ 里面有多少个被完全包含的区间，这个玩意儿也可以用区间 $dp$ 弄出来，也就是容斥一下：

$$ siz_{l, r} \; \text{+=} \; siz_{l + 1, r} + siz_{l, r - 1} - siz_{l + 1, r - 1} $$

&emsp; 然后对于一个 $k$，如果有 $siz_{l, k} + siz_{k +1, r} < siz_{l, r}$，那么就说明存在满足上述条件的区间，然后就是愉快的转移了。
...
&emsp; 于是我们可以考虑在枚举 $i$ 且处理 $\text{next}$ 之前的时候就先枚举一下 $&#124;p&#124;$ 做一个前缀和 $sum_k$ 表示满足关于 $i$ 对称的长度小于等于 $k$ 的字符串的数量。然后再记一个 $f$ 数组表示 $\text{border}$ 的前缀，然后每次加上最长的满足条件的 $\text{border}$ 的 $f$ 就好了，总复杂度是 $O(n^2)$ 的。
