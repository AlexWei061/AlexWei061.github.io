...
&emsp; 这个规律应该是很明显的了，如果我们把所有的 $i$ 位数（含前导0）中隔个数字出现的总次数记为 $f\_i$ 那么我们就有 $f\_i = 10 \times f\_{i-1} + 10^{i-1}$。（不知道怎么计算每个数字出现的次数的也可以试着打一下表，打表程序放在下面了）
...
3. 数字 0 出现的次数减去 $\sum\_{n=1}^{i-1}10^n$ 次。
