...
>$G(x) = a\_0 + a\_1x^1 + a\_2x^2 + a\_3x^3+\cdots$
...
&emsp; 这时候我们发现如果令 $a\_n = f\_n - \frac{1-\sqrt{5}}{2}f\_{n-1}$， $b\_n = f\_n - \frac{1+\sqrt{5}}{2}f\_{n-1}$，我们就得到了两个等比数列：
...
&emsp; 由等比数列的通项公式（$a\_n = a\_1r^{n-1}$ (这里的 r 是公
比)）我们可以得到：
...
&emsp; 我们设 F(n) 表示一个 1 $\times$ 2 的矩阵：$[fib\_n \quad fib\_{n+1}]$。然后我们再设另一个矩阵 A，我们希望 $F(n-1) \times A = F(n)$。那我们该如何设计这个矩阵呢，我们所希望得到的矩阵的第一个数是原矩阵的第二个数，而第二个数是原矩阵的两数之和。所以我们的 A 矩阵应该是一个 $2 \times 2$ 的矩阵，它长这样：
...
&emsp; 于是我们得到了一个矩阵乘法递推式，又因为矩阵乘法符合结合律，我们就可以得到一个用 $O(2^3logn)$ 的时间复杂度计算斐波那契数列的方法。首先我们设初始矩阵 F(0) = $[0 \quad 1]$。目标就是 F(n) = F(0) $\times A^n$。得到的矩阵的第一列的数就是 $fib\_n$ 了。
&emsp; 代码如下：
...
&emsp; 我们假设 $\frac{f\_n}{f\_{n+1}}$存在极限，且极限为 x。那么有：
