...
----
#### Stolz 定理
两个数列 $\lbrace x\_n \rbrace$ 和 $\lbrace y\_n \rbrace$，其中 $y\_n$ 是无穷大量。如果：
...
#### 夹逼定理
三个数列 $\lbrace x\_n \rbrace$ , $\lbrace y\_n \rbrace$ 和 $\lbrace z\_n \rbrace$。若：
...
4. 证明：若 $\lim\_{n\to \infty} a\_n = a$ 则 $\lim\_{n\to \infty}\frac{a\_1+a\_2+a\_3+\cdots + a\_n}{n} = a$
&emsp; 令 $x\_n = a\_1+\cdots+a\_n，y\_n = n$
...
5.  设 $a\_n > 0$， 且 $\lim\_{n\to \infty}a\_n = a$，证明：$\lim\_{n\to \infty}\sqrt[n]{a\_1a\_2\cdots a\_n} = a$
...
6. 若 $a\_n > 0$ （n = 1， 2， 3...），且 $\lim\_{n\to \infty}\frac{a\_{n+1}}{a\_n} = a$，求证 $\lim\_{n\to \infty}\sqrt[n]{a\_n} = a$


------
7. 求 $\lim\_{n\to \infty}\frac{1^2 + 3^3 + 5^2 + \cdots + (2n+1)^2}{n^3}$

-------

8. 用 Stolz 定理求 $\lim\_{n\to \infty}\frac{log\_an}{n}$ 和 $\lim\_{n\to \infty}\frac{n^k}{a^n}$
...
-----
9. 设 $\lbrace x\_n \rbrace$ 是无穷大量， $&#124;y\_n&#124; \geq \delta > 0$，证明 $\lbrace x\_ny\_n \rbrace$ 是无穷大量。
...
#### 题(们)
1.
&emsp; 当 a 为何值时，直线 $y = x$ 能与 $y = log\_ax$ 相切，切点在哪里。
...
&emsp; 所以当 $a = e^{\frac{1}{e}}$ 时， $y = log\_ax$ 与 $y=x$ 相切，切点为 $(e, e)$。
