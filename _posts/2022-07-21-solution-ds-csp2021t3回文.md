...
&emsp; 假设我们第一步是 $L$（第一步是 $R$ 其实也差不多， 这里就以是 $L$ 为例）。我们显然可以 $O(n)$ 在数组中找到一个点 $x$ 满足 $x \in [2, 2n] \land a_1 = a_x$。然后我们就可以发现，因为我们第一步把 $a_1$ 放到了 $b$ 数组的开头，所以 $b$ 数组的结尾就一定是我们找到的 $a_x$。所以我们就通过这两个点把数组分成了 $2$ 个栈，分别是：

1. **栈顶 $\to$ 栈底** : $a_2 \to a_{x - 1}$
2. **栈顶 $\to$ 栈底** : $a_{2n} \to a_{x + 1}$
